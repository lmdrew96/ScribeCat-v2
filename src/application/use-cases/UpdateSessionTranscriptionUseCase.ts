...
export class UpdateSessionTranscriptionUseCase {
  ...
  async execute(
    ...
    try {
      ...
      // IMPORTANT: Always save to local if found locally, so reloads get fresh data
      // Also save to cloud if it's a cloud session (for sync)
      try {
        if (foundLocally) {
