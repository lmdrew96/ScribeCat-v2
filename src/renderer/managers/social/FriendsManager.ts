 ...
 *
 * Delegates to:
 * - FriendsPresenceTracker: Presence tracking, heartbeat, activity updates
 * - FriendsRealtimeSubscriptions: Friend request real-time updates
 */
...
import { FriendsPresenceTracker } from './friends/FriendsPresenceTracker.js';
import { FriendsRealtimeSubscriptions } from './friends/FriendsRealtimeSubscriptions.js';
...
export class FriendsManager {
  ...
  // Delegated modules
  private presenceTracker: FriendsPresenceTracker;
  private realtimeSubscriptions: FriendsRealtimeSubscriptions;

  constructor() {
    // Initialize presence tracker with callbacks
    this.presenceTracker = new FriendsPresenceTracker({
      getCurrentUserId: () => this.currentUserId,
      getFriends: () => this.friends,
      updateFriends: (updater) => {
        this.friends = updater(this.friends);
      },
      notifyFriendsListeners: () => this.notifyFriendsListeners(),
    });

    // Initialize realtime subscriptions with callbacks
    this.realtimeSubscriptions = new FriendsRealtimeSubscriptions({
      getCurrentUserId: () => this.currentUserId,
      getFriendRequests: () => this.friendRequests,
      updateFriendRequests: (updater) => {
        this.friendRequests = updater(this.friendRequests);
      },
      addFriendRequest: (request) => {
        this.friendRequests.push(request);
      },
      notifyRequestsListeners: () => this.notifyRequestsListeners(),
    });
  ...
  async initialize(userId: string): Promise<void> {
    ...
    await this.presenceTracker.startPresenceTracking();

    // Subscribe to realtime friend request updates
    await this.realtimeSubscriptions.subscribeToRequests();
  ...
  async clear(): Promise<void> {
    ...
    await this.presenceTracker.stopPresenceTracking();

    // Unsubscribe from friend request updates
    this.realtimeSubscriptions.unsubscribeFromRequests();
    ...
    try {
      ...
      if (result.success) {
        ...
        if (this.currentUserId && this.friends.length > 0) {
          await this.presenceTracker.loadFriendsPresence();
        }
      } else {
  ...
  async setActivity(activity: string | undefined): Promise<void> {
    await this.presenceTracker.setActivity(activity);
  }

  /**
   * Stop presence tracking (for cleanup)
   */
